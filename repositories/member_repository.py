...
def update(member):
    sql = "UPDATE members SET (first_name, last_name, address, email, active) = (%s, %s, %s, %s, %s) WHERE id = %s"
    values = [member.first_name, member.last_name, member.address, member.email, member.active, member.id]
    run_sql(sql, values)
...
def select_all():
    ...
    results = run_sql(sql)
    for row in results:
        member = Member(row['first_name'], row['last_name'], row['address'], row['email'], row['active'], row['id'])
...
def select(id):
    ...
    member = Member(result['first_name'], result['last_name'], result['address'], result['email'], result['active'], id)
    return member
